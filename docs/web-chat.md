...
**Saved Builds (desktop):** Right **Arsenal** pane (≥861px) with folder chips, scrollable build cards (Warframe / weapons / companion / mods / arcanes / archon crystals), editable names, and **+ / −** controls. Add via the pane or `/save-build` (aliases `/savebuild`, `/arsenal-save`) — also browser `localStorage`. Favicon / PWA icons use `web/public/ordis-icon.svg` plus the PNG sizes next to it.
