...
3. For community build context, prefer local Overframe cache; if missing, ask yes/no before online Overframe/YouTube search (see recommend-build source policy).
4. Compare with a short table or bullets:
   - Damage profile
   - Crit vs status lean
...
5. Recommend one primary pick, plus when the runner-up wins.
6. Suggest one next step.
