/** Pure patch-detail helpers mirrored from `src/patches/detail.ts` for the web app. */

import { htmlToPlainText, truncatePlainText } from "@/lib/fetch-page";

export const PATCH_DETAIL_DEFAULT_MAX_CHARS = 12_000;
const PATCH_NOTES_URL = "https://www.warframe.com/en/patch-notes";
const PATCH_NOTES_PC_URL = "https://www.warframe.com/en/patch-notes/pc";
...
export type PatchDetailType = "Hotfix" | "Update" | "Other";
...
/** Same HTML→text conversion as `htmlToPlainText` (patch-page naming). */
export const htmlFragmentToPlainText = htmlToPlainText;

function classify(title: string): PatchDetailType {
...
export function parsePatchDetailHtml(
  ...
  const title = decodeEntities(
    (updateName ?? pageTitle ?? slug).replace(/^Warframe:\s*/i, "").trim(),
  );
  ...
  return {
    ...
    body,
    truncated,
    sectionHeadings: extractSectionHeadings(fullBody),
